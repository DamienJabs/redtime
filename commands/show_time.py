...
def build_time_table(redmine_url, redmine_api_key, user_id, date_start, date_end, month_date):
    total_days_month = calendar.monthrange(month_date.year, month_date.month)
    ...
    for i in range(1, total_days_month[1] + 1):
        format_data = datetime(month_date.year, month_date.month, i).strftime("%a %d")
        table.add_column(format_data)
        date = datetime(month_date.year, month_date.month, i).strftime("%Y-%m-%d")
        if date in total_time_day:
            if total_time_day[date] == 8:
                row.append(f"[green]{total_time_day[date]}[/green]")
            else:
                row.append(f"[yellow]{total_time_day[date]}[/yellow]")
        elif datetime(month_date.year, month_date.month, i).weekday() >= 5:
            row.append("[dim]-[/dim]")
        else:
            row.append("[red]0[/red]")

        if datetime(month_date.year, month_date.month, i).weekday() < 5:
            total_hours += 8

    total_time_spent = sum(total_time_day.values())
    table.add_column("Total", style="bold", width=10, justify="right")
    table.add_row(*row, f"{int(total_time_spent)}/{total_hours}")
...
def show_time(redmine_url, redmine_api_key, previous):
    ...
    month_date = datetime.now(tz).date()

    console = Console()
    
    if not previous:
        days = calendar.monthrange(month_date.year, month_date.month)
        date_start = datetime(month_date.year, month_date.month, 1).strftime("%Y-%m-%d")
        date_end = datetime(month_date.year, month_date.month, days[1]).strftime("%Y-%m-%d")

        table = build_time_table(redmine_url, redmine_api_key, user_id, date_start, date_end, month_date)

    else:
        last_day_prev = month_date.replace(day=1) - timedelta(days=1)
